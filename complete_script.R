...
mcs_longer <- mcs_nan %>% 
  pivot_longer(cols=c('APNGFE00_m','APCHSU00_m','APFASU00_m','APWKHA00_m','APCHUN00_m','APCHFA00_m'),
               names_to='variable',
               values_to='value')

ggplot(mcs_longer) +
  aes(y = variable, fill = factor(value)) +
  geom_bar(position = "likert") +
  scale_x_continuous(label = label_percent_abs()) +
  labs(
    x = "Proportion",
...
mcs_longer <- mcs_longer %>%
  mutate(
    variable = recode(variable,
      APWKHA00_m = "Mother and family happier if she goes to work",
      APCHSU00_m = "Child suffers if mother works before school",
      APFASU00_m = "Family life suffers when woman has full-time job",
      APCHUN00_m = "Mother and family happier if she goes to work",
      APCHSU00_m = "All right to have children without being married",
      APCHFA00_m = "Fathers’ involvement in upbringing"
    )
  )
